...
class VilleController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->request->get('newNom') && $request->request->get('newCp')){
            ...
            $ville->setNomVille($request->request->get('newNom'));
        ...
        if($request->request->get("nom") && $request->request->get("code postal")){

            $ville = new Ville();
            $ville->setNomVille($request->request->get("nom"));
            $ville->setCodePostal($request->request->get("code postal"));
            $em = $this->getDoctrine()->getManager();
            $em->persist($ville);
            $em->flush();
